...
const createAdminCommand = require('./command/createCommand/admin');
const interactionsCommand = require('./command/interactionsCommand');
const interactionsAdminCommand = require('./command/interactionsCommand/admin');
...
client.on("ready", async () => {
  ...
  const testGuildId = '972732510561763368';

  const guild = client.guilds.cache.get(guildId);
  const testGuild = client.guilds.cache.get(testGuildId)

  createCommand(client, testGuild);
  createAdminCommand(client, guild);
  
});

client.on('interactionCreate', async interaction => {
  if (!interaction.isCommand()) return;
  interactionsCommand(interaction);
  interactionsAdminCommand(interaction);
});
...
client.on("messageCreate", async (msg) => {
  ...
  // if (msg.author.id !== "779613987004219402") {
  //   if (msg.content.startsWith(callMe)) {
